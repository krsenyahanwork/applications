...
confusionMatrix(data = testProbs.hcv$resp.tree.synth, reference = testProbs.hcv$obs)
...
confusionMatrix(data = testProbs.fs$resp.tree.synth, reference = testProbs.fs$obs)
...
confusionMatrix(data = testProbs.logreg.fs$resp.logreg.synth, reference = testProbs.logreg.fs$obs)
...
confusionMatrix(data = testProbs.rf$resp.rf.synth, reference = testProbs.rf$obs)
...
confusionMatrix(data = testProbs.rf.hcv$resp.rf.synth, reference = testProbs.rf.hcv$obs)
...
confusionMatrix(data = testProbs.rf.transf$resp.rf.synth, reference = testProbs.rf.transf$obs)
...
```{r prediction for train and test}
pred.bestmodel.train = predict(rf_model.under.transf, newdata = train_data_transf, type = "prob")
pred.bestmodel.test = predict(rf_model.under.transf, newdata = test_data_transf, type = "prob")

tr_data.best <- train_data_transf
tr_data.best <- select(tr_data.best, -Target_fct)
tr_data.best$Pred.Proby.1 <- pred.bestmodel.train[,1]

ts_data.best <- test_data_transf
ts_data.best$Pred.Proby.1 <- pred.bestmodel.test[,1]

data_sample_transf.best <- rbind(tr_data.best, ts_data.best)
write.csv(data_sample_transf.best, '/Users/klemanroy/Downloads/applications/savii/data/data_sample_transf.best.csv')
```
...
#### Calibration of Probability Threshold
...
calibration_thresh <- function(data, algo_ = NA, approach_ = NA) {
  ...
  best_thresh <- best_thresh %>%
    mutate(prob_thresh = best_thresh$index/100,
           algorithm = algo_,
           approach = approach_) %>%
    select(algorithm, prob_thresh, model, approach, precision, recall, fmeasure)
...
dt.act <- calibration_thresh(tree.act, 'classification tree', 'act')
dt.hcv <- calibration_thresh(tree.hcv, 'classification tree', 'hcv')
dt.fs <- calibration_thresh(tree.fs, 'classification tree', 'fs')
dt.transf <- calibration_thresh(tree.transf, 'classification tree', 'transf')

lg.fs <- calibration_thresh(logreg.fs, 'logistic regression', 'fs')

rf.act <- calibration_thresh(randf.act, 'random forest', 'act')
rf.hcv <- calibration_thresh(randf.hcv, 'random forest', 'hcv')
rf.fs <- calibration_thresh(randf.fs, 'random forest', 'fs')
rf.transf <- calibration_thresh(randf.transf, 'random forest', 'transf')
